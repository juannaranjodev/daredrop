...
test('returns correct endpointId from module', () => {
	...
	expect(getEndpointIdFromModules(PENDING_PROJECTS_LIST_MODULE_ID)).toBe(GET_PENDING_PROJECTS)
	expect(getEndpointIdFromModules(ACTIVE_PROJECTS_LIST_MODULE_ID)).toEqual([GET_ACTIVE_PROJECTS, GET_PLEDGED_PROJECTS])
	expect(getEndpointIdFromModules(FAVORITES_PROJECTS_LIST_MODULE_ID)).toBe(GET_FAVORITES_LIST)
