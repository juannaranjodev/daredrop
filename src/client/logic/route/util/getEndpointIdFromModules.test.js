...
test('returns correct endpointId from module', () => {
  ...
  expect(getEndpointIdFromModules(ACTIVE_PROJECTS_LIST_MODULE_ID)).toBe(GET_ACTIVE_PROJECTS)
  expect(getEndpointIdFromModules(FAVORITES_PROJECTS_LIST_MODULE_ID)).toBe(GET_FAVORITES_LIST)
  expect(getEndpointIdFromModules(MY_PROJECTS_LIST_MODULE_ID)).toBe(GET_MY_PROJECTS)
})